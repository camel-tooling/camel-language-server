...
class CamelKModelineTraitHoverTest extends AbstractCamelLanguageServerTest {
	...
	void testProvideDocumentationOnHover() throws Exception {
		...
		HoverParams hoverParams = new HoverParams(new TextDocumentIdentifier(DUMMY_URI+".java"), new Position(0, 20));
		CompletableFuture<Hover> hover = camelLanguageServer.getTextDocumentService().hover(hoverParams);
		
		assertThat(hover.get().getContents().getLeft().get(0).getLeft()).isEqualTo("The Quarkus trait activates the Quarkus runtime.It's disabled by default.");
	}
